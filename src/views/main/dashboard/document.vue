  ...
  <div class="user-list-container">
    <el-card style="height: 100% !important">
      ...
      <el-table :data="users" stripe>
        ...
        <el-table-column prop="document_file" label="地址"></el-table-column>
      ...
      <template #footer>
        <el-form :model="createForm" ref="createFormRef" label-width="80px">
          ...
          <el-form-item label="名称">
            <el-input v-model="createForm.name" placeholder="请输入名称" clearable></el-input>
          </el-form-item>
    ...
    <!-- 更新对话框 -->
    <el-dialog v-model="updateDialogVisible" title="更新文档">
      <template #footer>
...
const resetForm=()=>{
    ...
    const fetchUsers = async () => {
      try {
        const response: any = await axios.get(`/api/documents?page=${currentPage.value}&limit=${pageSize.value}&name=${searchUsername.value}`, {
    });
    ...
    const handleFileChange = (file: any, fileLists: any) => {
      if (updateDialogVisible.value) {
        updateForm.value.document_file = file.raw; // 将文件保存到更新表单数据中
        updateForm.value.name = fileLists[0].name.replace(/\.[^.]*$/, ""); //版本号自动填入附件名称
        return file;
      } else {
        createForm.value.document_file = file.raw; // 将文件保存到创建表单数据中
    ...
    const submitCreateUser = async () => {
      try {
        ...
        if (createForm.value.document_file) {
          formData.append("document_file", createForm.value.document_file);
        }

        // 发送 POST 请求
        const response = await axios.post("/api/documents/create", formData, {
          headers: {
    ...
    const submitUpdateUser = async () => {
      try {
        ...
        if (updateForm.value.document_file) {
          formData.append("document_file", updateForm.value.document_file);
        }

        // 发送 PUT 请求
        const response = await axios.put("/api/documents/update", formData, {
          headers: {
            "Content-Type": "multipart/form-data", // 设置请求头
          },
        });
