  ...
  <div class="user-list-container">
    <el-card style="height: 100% !important">
      ...
      <el-table :data="users" stripe>
        ...
        <el-table-column prop="video_file" label="缩略图" align="center"
                            width="100" >
                          <!-- 默认加载图片：fallback="/path/to/default.jpg" -->
                          <template #default="scope">
                            <view class="imgOrvideoData-list">
                  <!-- <el-image v-if="dealFileType(scope.row.filePath).type == 'img'"
                    :src="scope.row.filePath"
                    style="width: 50px;height: 50px;text-align: center; border-radius: 5px;"></el-image> -->
                  <video 
                    :src="scope.row.video_file"
                    style="width: 50px;height: 50px;text-align: center;object-fit: cover; border-radius: 5px;"></video>
                  <!-- <span v-else>无</span> -->
                </view>
        ...
        <el-table-column prop="video_file" label="地址"></el-table-column>
...
const resetForm=()=>{
    ...
    const fetchUsers = async () => {
      try {
        const response: any = await axios.get(`/api/videos?page=${currentPage.value}&limit=${pageSize.value}&name=${searchUsername.value}`, {
    });
        
        users.value = response.data.videos;
    ...
    const submitUpdateUser = async () => {
      try {
        ...
        // 发送 PUT 请求
        const response = await axios.put("/api/videos/update", formData, {
          headers: {
            "Content-Type": "multipart/form-data", // 设置请求头
          },
        });
    ...
    const deleteUser = async (id: any) => {
      try {
        await axios.delete(`/api/videos/delete?id=${id}`);
